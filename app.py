# NAVLOG v18 — VFR híbrido + dog houses “compactas” + fix re-render
...
.mh{font-weight:800;font-size:28px;color:var(--mh)}
</style>
...
# ROC ref. (para posicionar TOC/TOD)
ROC_ENR = {
...
# ===================== MAPA (VFR) =====================
def house_polygon(lat, lon, heading_deg, w_nm=0.70, h_nm=0.90, roof_nm=0.28):
    # pentágono (base -> lado -> topo -> lado -> base)
    ...
    top_c_lat, top_c_lon = dest_point(lat, lon, heading_deg, h_nm/2.0)
    base_l_lat, base_l_lon = dest_point(left_lat, left_lon, heading_deg, -h_nm/2.0)
    base_r_lat, base_r_lon = dest_point(right_lat, right_lon, heading_deg, -h_nm/2.0)
    roof_lat, roof_lon = dest_point(top_c_lat, top_c_lon, heading_deg, roof_nm)
    ...
    return [
        [base_l_lon, base_l_lat],
        [base_r_lon, base_r_lat],
        [top_r_lon, top_r_lat],
        [roof_lon, roof_lat],
        [top_l_lon, top_l_lat],
        [base_l_lon, base_l_lat]
...
def chip_rect_centered(lat, lon, heading_deg, w_nm, h_nm, forward_nm):
    # rect centrado no eixo da leg (para caber DENTRO da casa)
...
def tile_layer(urls, opacity=1.0, max_zoom=19):
    return pdk.Layer("TileLayer", data=urls if isinstance(urls, list) else [urls],
...
def render_map_vfr():
    ...
    try: default_index = styles.index(st.session_state.get("map_style", styles[0]))
    ...
    HOUSE_OFFSET_NM = 0.60  # afastar da rota
    MH_SIZE, INF_SIZE = 30, 16

    under_paths, over_paths, tick_data = [], [], []
    houses, chips, mh_labels, info_labels = [], [], [], []
    wp_points, wp_texts, special_points, special_texts, inner_lines = [], [], [], [], []
    ...
    for s in st.session_state.sublegs:
        ...
        # 2 caixinhas internas: MH (cima) + INFO (baixo)
        chips.append({"polygon": chip_rect_centered(off_lat, off_lon, s["TC"], w_nm=0.55, h_nm=0.22, forward_nm= 0.12)})
        chips.append({"polygon": chip_rect_centered(off_lat, off_lon, s["TC"], w_nm=0.55, h_nm=0.22, forward_nm=-0.10)})

        # “linhas” internas (estética — como no teu exemplo)
        for offset in (0.12, -0.10):
            c_lat, c_lon = dest_point(off_lat, off_lon, s["TC"], offset)
            l1a = dest_point(c_lat, c_lon, s["TC"]-90, 0.22)
            l1b = dest_point(c_lat, c_lon, s["TC"]+90, 0.22)
            l2a = dest_point(c_lat, c_lon, s["TC"]-90, 0.10)
            l2b = dest_point(c_lat, c_lon, s["TC"]+90, 0.10)
            inner_lines.append({"path":[[l1a[1],l1a[0]],[l1b[1],l1b[0]] ]})
            inner_lines.append({"path":[[l2a[1],l2a[0]],[l2b[1],l2b[0]] ]})

        # textos nas caixinhas
        mh_text = f"{rang(s['MH'])} M"
        info = f"{rang(s['TC'])} T   {r10f(s['Dist'])} nm   GS {rint(s['GS'])}   ETE {mmss(s['ETE'])}" + (f"   ETO {s['ETO']}" if s.get("ETO") else "")
        pos_mh   = dest_point(off_lat, off_lon, s["TC"],  0.12)
        pos_info = dest_point(off_lat, off_lon, s["TC"], -0.10)
        mh_labels.append(  {"position":[pos_mh[1],   pos_mh[0]],   "text": mh_text})
        info_labels.append({"position":[pos_info[1], pos_info[0]], "text": info})

        # TOC/TOD (marcados fora da linha)
        if "TOC" in s:
            lat_toc, lon_toc = s["TOC"]
            special_points.append({"position":[lon_toc, lat_toc], "name":"TOC"})
            tocl = dest_point(lat_toc, lon_toc, s["TC"]+100, 0.18)
        ...
        if "TOD" in s:
            ...
            todl = dest_point(lat_tod, lon_tod, s["TC"]+100, 0.18)
            special_texts.append({"position":[todl[1], todl[0]], "text":"TOD"})

    # camadas de fundo (URLs explícitos + subdomínios) — estilos VFR
    layers = []
    if current_style == "VFR híbrido (OTM + OSM labels)":
        layers.append(tile_layer(
            ["https://a.tile.opentopomap.org/{z}/{x}/{y}.png",
             "https://b.tile.opentopomap.org/{z}/{x}/{y}.png",
             "https://c.tile.opentopomap.org/{z}/{x}/{y}.png"], opacity=1.0, max_zoom=17))
        layers.append(tile_layer(
            ["https://a.tile.openstreetmap.org/{z}/{x}/{y}.png",
             "https://b.tile.openstreetmap.org/{z}/{x}/{y}.png",
             "https://c.tile.openstreetmap.org/{z}/{x}/{y}.png"], opacity=0.55))
    if current_style == "OSM HOT (labels grandes)":
        layers.append(tile_layer(
            ["https://a.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png",
             "https://b.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png",
             "https://c.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png"], opacity=1.0))
    ...
    if current_style == "Esri Topo":
        layers.append(tile_layer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer/tile/{z}/{y}/{x}", opacity=1.0))
    if current_style == "Satélite híbrido (Esri + OSM)":
        layers.append(tile_layer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}", opacity=1.0))
        layers.append(tile_layer(
            ["https://a.tile.openstreetmap.org/{z}/{x}/{y}.png",
             "https://b.tile.openstreetmap.org/{z}/{x}/{y}.png",
             "https://c.tile.openstreetmap.org/{z}/{x}/{y}.png"], opacity=0.35))
    ...
    layers += [
        ...
        pdk.Layer("PathLayer", data=tick_data,   get_path="path", get_color=[0,0,0,255],     
