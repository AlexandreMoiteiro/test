# app.py — NAVLOG v12.2 — Dog Houses triangulares + Fixes/TOC/TOD marcados + seleção 1-a-1
...
# -------------------- PAGE / CSS --------------------
st.set_page_config(page_title="NAVLOG v12.2 — TOC/TOD + Dog Houses", layout="wide")
st.markdown("""
<style>
...
RPM_CLIMB, RPM_CRUISE, RPM_DESC = 2250, 2100, 1800  # só para FF (interno)
    ...
    if profile == "CLIMB":
        ...
        if d_need <= dist:
            ...
            if rem > 0:
                ...
                segs.append({"name":"Cruise (após TOC)","TH":THr,"MH":MHr,"GS":GScr,"TAS":TAS_CRUISE,"ff":FF_cru,"time":tB,"dist":rem,"alt0":alt1,"alt1":alt1})
...
with h1: st.title("NAVLOG — v12.2 (AFM)")
...
try:
    ...
    st.warning("Não foi possível ler os CSVs locais. Verifica AD-HEL-ULM.csv e Localidades-*.csv.")

f1,f2,f3 = st.columns([3,3,2])
with f1: qtxt = st.text_input("🔎 Procurar (código/ident/nome)", "", placeholder="Ex: NISA, LP0078, ABRANTES…")
...
label_opts = [f"{k} — {c} — {n}  ({lat:.4f}, {lon:.4f})" for (k,c,n,lat,lon) in options]
with f3:
    idx_sel = st.selectbox("Resultado", options=list(range(len(label_opts))), format_func=lambda i: label_opts[i] if label_opts else "—", disabled=(len(label_opts)==0))
...
if len(st.session_state.route_points) >= 2 and st.session_state.legs and st.session_state.computed_by_leg:
    ...
    deck = pdk.Deck(
        map_style="https://basemaps.cartocdn.com/gl/voyager-gl-style/style.json",  # nomes de terras + características
