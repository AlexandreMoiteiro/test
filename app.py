# app.py — NAVLOG (Leaflet/Folium) — VFR + TOC/TOD + Print-friendly
# - TAS fixas: 70 kt (climb), 90 kt (cruise/descida)
# - FF fixa: 20 L/h
# - TOC/TOD são novos waypoints; legs partidas nesses pontos
# - Mapa Folium/Leaflet: bases VFR (satélite/topo) estáveis
# - Dog houses triangulares; MH grande (amarelo) com halo
# - Riscas a cada 2 min pela GS
# - Seletor de pontos com st.data_editor (checkbox por linha)
# - Botão de imprimir + "Print scale" para não perder legibilidade no PDF
...
ens("maptiler_key", "")
ens("print_scale", 1.15)  # multiplicador de tamanhos quando fores imprimir

# =============== HEADER ===============
st.markdown("<div class='sticky'>", unsafe_allow_html=True)
h1, h2, h3, h4 = st.columns([3,3,2,2])
with h1: st.title("NAVLOG — Folium (VFR) + TOC/TOD")
with h2: st.caption("TAS 70/90/90 · FF 20 L/h · motor Leaflet estável · PDF amigável")
with h3:
...
with st.form("globals"):
    ...
    with b3:
        st.session_state.print_scale = st.slider("Print scale (↑ aumenta texto/linhas)", 1.0, 2.0, float(st.session_state.print_scale), 0.05)
...
def parse_ad_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for line in df.iloc[:,0].dropna().tolist():
        ...
        if len(coord_toks) >= 2:
            ...
            rows.append({"SRC":"AD","CODE":ident or name, "NAME":name, "CITY":city,"LAT":lat,"LON":lon})
...
def parse_loc_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for line in df.iloc[:,0].dropna().tolist():
        ...
        if len(coord_toks) >= 2:
            ...
            rows.append({"SRC":"LOC","CODE":code or name, "NAME":name, "SECTOR":sector,"LAT":lat,"LON":lon})
    return pd.DataFrame(rows).dropna(subset=["LAT","LON"])
...
try:
    ...
    loc_df = pd.DataFrame(columns=["SRC","CODE","NAME","SECTOR","LAT","LON"])
    st.warning("Não foi possível ler os CSVs locais. Verifica os nomes de ficheiro.")

# =============== PESQUISA + SELETOR BONITO (data_editor) ===============
...
if not results.empty:
    ...
    df_show = df_show[["ADD","SRC","CODE","NAME","CITY","SECTOR","LAT","LON"]].fillna("")
    edited = st.data_editor(
        ...
        column_config={
            ...
            "SRC": st.column_config.TextColumn("Origem", disabled=True),
            "CODE": st.column_config.TextColumn("Código", disabled=True),
            "NAME": st.column_config.TextColumn("Nome", disabled=True),
            "CITY": st.column_config.TextColumn("Cidade", disabled=True),
            "SECTOR": st.column_config.TextColumn("Sector", disabled=True),
            "LAT": st.column_config.NumberColumn("Lat", format="%.5f", disabled=True),
            "LON": st.column_config.NumberColumn("Lon", format="%.5f", disabled=True),
        },
        height=320
    )
...
def render_map_folium(nodes, legs, base_choice, maptiler_key, print_scale=1.15):
    ...
    if base_choice == "OpenTopoMap (topo VFR-ish)":
        ...
        folium.TileLayer("https://tiles.maps.eox.at/wmts/1.0.0/s2cloudless-2020_3857/default/g/{z}/{y}/{x}.jpg",
                         attr="© EOX & Sentinel-2 cloudless", name="EOX Sat").add_to(m)
        folium.TileLayer("https://tiles.maps.eox.at/wmts/1.0.0/overlay_bright/GoogleMapsCompatible/{z}/{y}/{x}.png",
                         attr="© EOX overlay", name="Labels", overlay=True, control=True, opacity=1.0).add_to(m)
        ...
        folium.TileLayer("https://services.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}",
                         attr="© Esri Places", name="Places", overlay=True, control=True).add_to(m)
        ...
        folium.TileLayer("https://services.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer/tile/{z}/{y}/{x}",
                         attr="© Esri", name="ESRI WorldTopo").add_to(m)
        ...
        folium.TileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",
                         attr="© OpenStreetMap contributors", name="OSM Standard").add_to(m)

    # ---------- DESENHO (com escala de impressão) ----------
    s = float(print_scale)  # multiplicador para PDF (mais legível quando o mapa é abrangente)
    ...
    for L in legs:
        ...
        folium.PolyLine(latlngs, color="#FFFFFF", weight=8*s, opacity=0.95).add_to(m)
        folium.PolyLine(latlngs, color="#C000FF", weight=4*s, opacity=1.0).add_to(m)
    ...
    for L in legs:
        ...
        while k*step <= L["time_sec"]:
            ...
            folium.PolyLine([(left_lat,left_lon),(right_lat,right_lon)], color="#000000", weight=2*s, opacity=1).add_to(m)
            k += 1
    ...
    for L in legs:
        mid_lat, mid_lon = point_along_gc(L["A"]["lat"], L["A"]["lon"], L["B"]["lat"], L["B"]["lon"], L["Dist"]/2.0)
        off_lat, off_lon = dest_point(mid_lat, mid_lon, L["TC"]+side*90, 0.40)
        _add_text(m, off_lat, off_lon, f"MH {rang(L['MH'])}°", size_px=int(28*s), color="#FFD700", offset_px=(0,int(-4*s)), halo=True)
        info = f"{rang(L['TH'])}T • {rint(L['GS'])}kt • {mmss(L['time_sec'])} • {L['Dist']:.1f}nm"
        _add_text(m, off_lat, off_lon, info, size_px=int(14*s), color="#000000", offset_px=(int(side*92*s),int(18*s)), halo=True)
        side *= -1
    ...
    for idx, N in enumerate(nodes):
        ...
        folium.CircleMarker((N["lat"],N["lon"]), radius=6*s, color="#FFFFFF", weight=2*s,
                            fill=True, fill_opacity=1, fill_color=color).add_to(m)
        _add_text(m, N["lat"], N["lon"], f"{idx+1}. {N['name']}", size_px=int(14*s), color="#FFFFFF", offset_px=(0,int(-22*s)), halo=True)

    # Controlo + botão imprimir + ajustar vista com padding
    folium.LayerControl(collapsed=False).add_to(m)
    enable_print(m)
    try:
        sw, ne = _bounds_from_nodes(nodes)
...
if st.session_state.wps:
    if not st.session_state.route_nodes:
        ...
        render_map_folium(
            ...
            print_scale=st.session_state.print_scale
        )
else:
