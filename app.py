...
def init_state():
    ...
    if "last_click" not in st.session_state:
        st.session_state.last_click = None  # Optional[Tuple[float, float]]
    if "last_registered" not in st.session_state:
        st.session_state.last_registered = None  # Optional[Tuple[float, float]]
...
def draw_overlay(img: Image.Image) -> Image.Image:
    ...
    for p, pts in st.session_state.panel_corners.items():
        if len(pts) == 4:
            d.polygon(pts, outline=(0, 200, 255), width=5)
            d.text((pts[0][0] + 8, pts[0][1] + 8), p, fill=(0, 200, 255))

    # ticks (laranja)
    for axis_name, ticks in st.session_state.axis_ticks.items():
        for t in ticks:
            x, y = t["x"], t["y"]
            d.ellipse((x - 8, y - 8, x + 8, y + 8), outline=(255, 165, 0), width=4)
            d.text((x + 10, y - 10), f'{axis_name}:{t["value"]}', fill=(255, 165, 0))

    # lines (vermelho)
    for k, segs in st.session_state.lines.items():
        for seg in segs:
            d.line([(seg["x1"], seg["y1"]), (seg["x2"], seg["y2"])], fill=(255, 0, 0), width=4)
...
def register_click(x: float, y: float):
    ...
    st.session_state.last_registered = (x, y)
...
with st.sidebar:
    ...
    if st.session_state.mode == "panel":
        ...
        st.caption("Clica 4 cantos: TL → TR → BR → BL")
        ...
        st.caption("Clica 2 pontos para criar 1 segmento.")
        c1, c2 = st.columns(2)
        with c1:
...
with col2:
    st.subheader("Estado")
    st.write("Último clique registado:", st.session_state.last_registered)
    st.write("Ponto pendente (line mode):", st.session_state.pending_point)
