...
# ===================== MAPA (VFR) =====================
# Escolha do basemap (estilo VFR)
base = st.selectbox(
    "Base map (VFR)",
    ["OpenTopoMap (VFR-like)", "OSM Standard", "CARTO Voyager"],
    index=0,
    help="OpenTopoMap dá relevo/curvas de nível; OSM Standard tem mais detalhe urbano; CARTO Voyager é claro."
)

if len(points) >= 2 and sublegs:
    # camadas
    base_layers = []
    if base == "OpenTopoMap (VFR-like)":
        ...
        # Hillshade extra (suave)
        base_layers.append(pdk.Layer(
            "TileLayer",
            data="https://tiles.wmflabs.org/hillshading/{z}/{x}/{y}.png",
            min_zoom=0,
            max_zoom=17,
            tile_size=256,
            opacity=0.35,
        ...
        base_layers.append(pdk.Layer(
            "TileLayer",
            data="https://{a-c}.tile.openstreetmap.org/{z}/{x}/{y}.png",
            min_zoom=0,
            max_zoom=19,
            tile_size=256,
            opacity=1.0,
    ...
    else:  # CARTO Voyager vector -> usar raster fallback
        base_layers.append(pdk.Layer(
            "TileLayer",
    ...
    for p in points:
        ...
        if str(p["name"]).upper().startswith("TOC") or str(p["name"]).upper().startswith("TOD"):
            todtoc_points.append({"position":[p["lon"], p["lat"]]})
            todtoc_labels.append({"position":[p["lon"], p["lat"]], "text": p["name"], "size": 12})
    ...
    for s in sublegs:
        ...
        tri = triangle_coords(off_lat, off_lon, s["TC"], h_nm=1.0, w_nm=0.75)
        ...
        lab_lat, lab_lon = dest_point(off_lat, off_lon, s["TC"]+90, 0.40)
        info = f"{rang(s['TH'])}T • {s['dist']:.1f}nm • GS {rint(s['GS'])} • ETE {mmss(s['time_s'])}"
        info_labels.append({"position":[lab_lon, lab_lat], "text": info, "size": 14})

    # Graticule (lat/lon) para navegação visual — passo de 0.25°
    ...
    g = lat_min
    while g <= lat_max:
        grat_paths.append({"path": [[lon_min, g], [lon_max, g]]})
        g += step
    g = lon_min
    while g <= lon_max:
        grat_paths.append({"path": [[g, lat_min], [g, lat_max]]})
        g += step
    graticule_layer = pdk.Layer("PathLayer", data=grat_paths, get_path="path", get_color=[180,180,180,120], width_min_pixels=1)

    # Layers vetoriais
    route_layer = pdk.Layer("PathLayer", data=path_data, get_path="path", get_color=[180, 0, 255, 230], width_min_pixels=5)
    ...
    todtod_text  = pdk.Layer(
        "TextLayer", data=todtoc_labels, get_position="position", get_text="text", get_size="size", get_color=[0,0,0]
    )
    ...
    view_state = pdk.ViewState(latitude=mean_lat, longitude=mean_lon, zoom=8.0, pitch=0)

    deck = pdk.Deck(
        map_style=None,
        initial_view_state=view_state,
        layers=base_layers + [graticule_layer, route_layer, ticks_layer, tri_layer, wp_layer, wp_text_layer, todtod_layer, todtod_text, mh_layer, info_layer],
        tooltip={"text": "{name}"}
    )
